...
class DataGenerator():
    def __init__(self, batch_size=32, vad_mode=None, data_list=None, mode=1):
        ...
        self.mode = mode
        self.num_classes = 10
        self.audio_path = 'audio/audio_train/'
...
#         self.test_durations = s_durations[train_size::]
    ...
    def get_test(self):
        ...
        if self.mode == 1:
            ...
            features = self.gen_delta_delta(self.test_fns)
        else:
            features = self.gen_filtered_spec(self.test_fns)
...
class AudioGenerator():
    ...
    def get_test(self):
        ...
        self.shuffle_data_by_partition()
        if self.mode == 1:
            features = self.gen_spectrogram(self.train_fns)
        elif self.mode == 2:
            features = self.gen_delta_delta(self.train_fns)
        else:
            features = self.gen_filtered_spec(self.train_fns)

        texts = np.argmax(self.train_labels, axis=1)
